...
public class CustomCompositeKnowledgeBuilderImpl implements CompositeKnowledgeBuilder {
    ...
    public CustomCompositeKnowledgeBuilderImpl( KnowledgeBuilderImpl kBuilder ) {
    ...
    private void buildRules( Collection<CompositePackageDescr> packages ) {
        int count = 1;
        for ( CompositePackageDescr packageDescr : packages ) {
            String format = "Building package : %s | %d of %d";
            System.out.println( String.format( format, packageDescr.getName(), count, packages.size() ) );
            kBuilder.setAssetFilter( packageDescr.getFilter() );
            PackageRegistry pkgRegistry = kBuilder.getPackageRegistry( packageDescr.getNamespace() );
